...
" Sync config
"let s:server = 'heisoo.gicp.net'
"let s:path   = '/mnt/heisoo/projects/dev/tuangoso/trunk/codes/frontend/'
"let s:username = 'www-data'
"let s:password = '!@W_heisoo'
"let s:port     = 21
...
function! g:PJGoToModule(PJRoot, type)
    ...
    let viewRoot    = a:PJRoot . 'application/views/templates/'
...
function! s:OtherConfig()
    ...
    " redefind path
    let &path = '.'
    if finddir('views', s:PJRoot . 'application') != ''
        exec 'set path+=' . s:Slash(s:PJRoot)
        exec 'set path+=' . s:Slash(s:PJRoot . 'application\views\')
    endif
    ...
    if exists('s:ToPage') && s:ToPage != ''
        if finddir('views', s:PJRoot . 'application') != ''
            exec 'nmap '.s:ToPage.' :NERDTree '.s:PJRoot.'application/views/<CR>'
        else
    ...
    if exists('s:ToCurrentPresenter') && s:ToCurrentPresenter != ''
        if finddir('application', s:PJRoot . 'application/') != ''
            let filename = substitute(expand('%:t:r'), '\w*', '\u&', 'g')
            exec 'nmap '.s:ToCurrentPresenter.' :tabnew '.s:PJRoot.'application/controllers/'.filename.'.php<CR>'
        endif
    endif
    ...
    if exists('s:ToService') && s:ToService != ''
        if finddir('libraries', s:PJRoot. 'application') != ''
            exec 'nmap '.s:ToService.' :NERDTree '.s:PJRoot.'application/libraries/<CR>'
        else
    ...
    if exists('s:ToModuleHtm') && s:ToModuleHtm != ''
        exec 'nmap ' . s:ToModuleHtm . " :call g:PJGoToModule('".s:PJRoot."', 'tpl')<CR>"
    endif
    ...
    "to model
    if exists('s:ToModel') && s:ToModel != ''
        if finddir('models', s:PJRoot.'application/') != ''
            "let file = substitute(expand('%:t:r'), 'Controller', '', 'g')
            let modelName  = expand('<cfile>:r')
            exec 'nmap '.s:ToModel.' :tabnew '.s:PJRoot.'application/models/'.modelName.'.php<CR>'
        endif
    endif
endfunction
"}}}
